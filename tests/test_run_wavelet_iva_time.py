"""
Tests for scripts/run_wavelet_iva_time.py — the time-as-mixing IVA CLI.
...
temporal pattern and a ``(frequencies, channels)`` score map, and **no**
time-frequency map, which only the channel-as-mixing variant produces — and that it
refuses to store anything it could not label by participant.
"""
...
from scripts.run_wavelet_iva_time import _run_iva  # noqa: E402
...
class TestComponentStore:
    ...
    def test_stores_the_products_this_variant_actually_has(
        ...
        # Time is on the mixing axis here, so there is no TF map to store.
