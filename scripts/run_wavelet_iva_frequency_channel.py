...
layout under::
    ...
    plots/05-wavelet-iva-analysis/<Condition>_<MusicType>/
        broadband/iva_frequency_channel/              # default (no --band)
            pca_scree_<label>.png
            ...
        bands/iva_frequency_channel/                  # when --band <name> is given
...
def _plot_pairmap_time_subject(
    ...
    *,
    ...
    axes[-1].set_xlabel("Time (s)")
    fig.suptitle(
        f"Time × Subject per IC (from scores) — {label}", fontsize=13, y=1.01
    )
    fig.tight_layout()
    fig.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def _run_iva(
    ...
    *,
    ...
    if band is None:
        out_dir = save_dir / "broadband" / "iva_frequency_channel"
        prefix = ""
    else:
        out_dir = save_dir / "bands" / "iva_frequency_channel"
        prefix = f"{band}_"
    ...
    source_corr = np.stack(
        [np.corrcoef(iva_scores_pca[:, k, :]) for k in range(n_show)]
    )
    _plot_isc_grid(
        source_corr,
        fig_title=f"Intersubject Correlation of IVA Score Timecourses — {label}",
        save_path=out_dir / f"{prefix}iva_score_isc_time_{label}.png",
    )
    ...
    _plot_isc_grid(
        pattern_corr,
        fig_title=f"Intersubject Correlation of IVA Component (F × C) Patterns — {label}",
        save_path=out_dir / f"{prefix}iva_pattern_isc_freq_channel_{label}.png",
    )

    # (d) Per-IC LOO-ISC bar
    subj_time_view = iva_scores_pca[:, :n_show, :].transpose(1, 0, 2)  # (K, S, T)
    _plot_loo_isc_bar(
        subj_time_view,
        n_show=n_show,
        label=label,
    ...
    chan_loading = iva_components[:, :n_show, :, :].mean(axis=2)  # (S, n_show, C)
    if info is not None:
    ...
    mean_components = iva_components.mean(axis=0)  # (N_PCA, F, C)
    freq_profiles = mean_components[:n_show].mean(axis=2).T  # (F, n_show)
    time_profiles = iva_scores_pca.mean(axis=0)[:n_show]  # (n_show, T)
    _plot_tf_map(
        freq_profiles,
        time_profiles,
        time,
    ...
    flat_patterns = iva_components[:, :n_show, :, :].reshape(
        n_subjects, n_show, -1
    ...
    time_subj = iva_scores_pca.transpose(1, 0, 2)[:n_show]  # (K, S, T)
    ...
    subj_freq = iva_components.mean(axis=-1).transpose(1, 2, 0)[:n_show]  # (K, F, S)
    ...
    subj_chan = iva_components.mean(axis=-2).transpose(1, 2, 0)[:n_show]  # (K, C, S)
    _plot_pairmap_subject_x(
        subj_chan,
    ...
    freq_chan = iva_components.mean(axis=0)[:n_show]  # (K, F, C)
    ...
    subject_loadings = np.abs(iva_scores_pca[:, :n_show, :]).mean(axis=2)  # (S, n_show)
    ...
    _logger.info(
        f"[{label}] IVA-frequency-channel: 10+ plot files saved to {out_dir}"
    )
