...
def _run_iva(
    ...
    *,
    ...
    if band is None:
        out_dir = save_dir / "broadband" / "iva_frequency_channel" / pca_subdir
        prefix = ""
    else:
        out_dir = save_dir / "bands" / "iva_frequency_channel" / pca_subdir
        prefix = f"{band}_"
    ...
    for indices, labels_grp, group_name, suffix in (
        ...
        source_corr = np.stack(
            [np.corrcoef(iva_scores_pca[:, k, :]) for k in indices]
        )
        _plot_isc_grid(
            source_corr,
            labels=labels_grp,
    ...
    for indices, labels_grp, group_name, suffix in (
        ...
        _plot_isc_grid(
            ...
            save_path=out_dir / f"{prefix}iva_pattern_isc_freq_channel_{label}_{suffix}.png",
        )
    ...
    flat_patterns = iva_components[:, selected_indices, :, :].reshape(
        n_subjects, n_show, -1
    ...
    subject_loadings_all = np.abs(iva_scores_pca).mean(axis=2)  # (S, N_PCA) — mean |score| over T
    for indices, labels_grp, suffix in (
        (top_indices, top_labels, "top"),
        (bottom_indices, bot_labels, "bottom"),
    ):
    ...
    _logger.info(
        f"[{label}] IVA-frequency-channel: 20+ plot files saved to {out_dir}"
    )
