...
(subjects-as-datasets, frequency × channel is the independent dimension:
K = S, features = C × F, samples = T; per-subject PCA → IVA-G).
...
layout under::
    ...
    plots/05-frequency-channel-wavelet-iva-analysis/<Condition>_<MusicType>/
        broadband/iva_frequency_channel/pca_<n_pca>/  # default (no --band)
            pca_scree_<label>.png
            iva_component_ranking_<label>.png
            iva_score_isc_time_<label>_top.png
            iva_score_isc_time_<label>_bottom.png
            ...
        bands/iva_frequency_channel/pca_<n_pca>/      # when --band <name> is given
...
Usage::
    ...
    # broadband
    python scripts/run_wavelet_iva_frequency_channel.py \\
...
``data/processed/<experiment>/iva_results/<Condition>/``, with the participant label
of every row, and read them back with
:func:`src.io.iva_store.load_iva_components`. This variant puts frequency on the
mixing axis, so what is stored is the component timecourse and its
``(frequencies, channels)`` pattern — there is no per-component TF map here; the
channel-as-mixing variant (``run_wavelet_iva_channel.py``) is the one that has one.
"""
...
_STAGE_DIR = "05-frequency-channel-wavelet-iva-analysis"
_ANALYSIS_DIR = IvaVariants.FREQUENCY_CHANNEL.value
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the (frequency × channel)-as-independent IVA decomposition on wavelet "
...
def _plot_loo_isc_bar(
    ...
    *,
    ...
    ax.set_title(f"Per-IC Mean LOO-ISC — Time Dimension (score timecourses) — {label}")
...
def _plot_topomap_mean_var(
    ...
    *,
    ...
    fixes the source scale but not the pattern scale, so unnormalised maps
    differ across subjects by a subject-specific gain that would let the loudest
...
def _plot_pairmap_time_subject(
    ...
    *,
    ...
    axes[-1].set_xlabel("Time (s)")
    fig.suptitle(
        f"Time × Subject per IC (from scores) — {label}",
        fontsize=13,
        y=1.01,
    )
    fig.tight_layout()
...
def _plot_pairmap_frequency_channel(
    ...
    *,
    ...
    fig.suptitle(
        f"Frequency × Channel per IC (subject-avg components) — {label}",
        fontsize=13,
        y=1.02,
    )
    fig.tight_layout()
...
# ---------------------------------------------------------------------------
# Main IVA pipeline (subjects-as-datasets; (F × C) as the independent axis)
# ---------------------------------------------------------------------------
...
def _run_iva(
    ...
    *,
    ...
    _logger.info(f"[{label}] IVA-frequency-channel: {data_4d.shape}  sfreq={sfreq} Hz")
    ...
    X_subjects = bb_z.reshape(n_subjects, n_feat, n_times)
    _logger.info(f"[{label}] Per-subject reshape: {X_subjects.shape}  (S, C*F, T)")

    # Step 2 — Per-subject PCA → IVA input (N_PCA, T, S).
    pcas: list[PCA] = []
    pca_scores = np.zeros((n_subjects, n_pca, n_times))
    pca_evr = np.zeros((n_subjects, n_pca))
    for k in range(n_subjects):
        subj_matrix = X_subjects[k].T  # (T, C*F)
    ...
    iva_scores_pca = np.zeros((n_subjects, n_pca, n_times))
    iva_components = np.zeros((n_subjects, n_pca, n_freqs, n_channels))
    for k in range(n_subjects):
        W_k = W[:, :, k]
    ...
    _logger.info(
        f"[{label}] Recover: iva_scores_pca={iva_scores_pca.shape}, "
    ...
    # Store the recovered components before drawing anything: the arrays are the
    # expensive product, and a figure failing on a headless node should not cost the
    # decomposition. The participant labels go in with them — without the
    # row-to-participant mapping the subject axis of these arrays is unreadable.
    # This variant puts channel x frequency on the mixing axis and time on the
    # sample axis, so a component owns a spectro-spatial pattern and a timecourse
    # — there is no per-component time-frequency map to store here; the
    # channel-as-mixing variant (run_wavelet_iva_channel.py) is the one that has
    # one.
    if store_root is not None:
        ...
        channel_names = (
            list(topo_info_subset(info, n_channels).ch_names)
            if info is not None
            else None
        )
        save_iva_components(
            ...
            arrays={
                IvaComponentArrays.FREQUENCY_CHANNEL_PATTERN: iva_components,
                IvaComponentArrays.TIMECOURSE: iva_scores_pca,
            },
            freqs=freqs,
            times=time,
    ...
    for indices, labels_grp, group_name, suffix in (
        ...
        source_corr = np.stack([np.corrcoef(iva_scores_pca[:, k, :]) for k in indices])
        _plot_isc_grid(
            source_corr,
            labels=labels_grp,
            fig_title=(
            ...
            save_path=out_dir / f"{prefix}iva_score_isc_time_{label}_{suffix}.png",
        )
    ...
    for indices, labels_grp, group_name, suffix in (
        ...
        for i, k in enumerate(indices):
            flat = iva_components[:, k, :, :].reshape(n_subjects, -1)
        ...
        _plot_isc_grid(
            ...
            save_path=out_dir
            / f"{prefix}iva_pattern_isc_freq_channel_{label}_{suffix}.png",
        )
    ...
    subj_time_view = iva_scores_pca[:, selected_indices, :].transpose(1, 0, 2)
    _plot_loo_isc_bar(
        subj_time_view,
    ...
    chan_loading_all = iva_components.mean(axis=2)  # (S, N_PCA, C)
    if info is not None:
    ...
    mean_components_all = iva_components.mean(axis=0)  # (N_PCA, F, C)
    time_profiles_all = iva_scores_pca.mean(axis=0)  # (N_PCA, T)
    for indices, labels_grp, suffix in (
        (top_indices, top_labels, "top"),
        (bottom_indices, bot_labels, "bottom"),
    ):
    ...
    flat_patterns = (
        iva_components[:, selected_indices, :, :]
    ...
    for indices, labels_grp, suffix in (
        ...
        time_subj = iva_scores_pca.transpose(1, 0, 2)[indices]  # (K, S, T)
        ...
        subj_freq = iva_components.mean(axis=-1).transpose(1, 2, 0)[indices]
        ...
        subj_chan = iva_components.mean(axis=-2).transpose(1, 2, 0)[indices]
        _plot_pairmap_subject_x(
            subj_chan,
        ...
        freq_chan = iva_components.mean(axis=0)[indices]  # (K, F, C)
    ...
    subject_loadings_all = np.abs(iva_scores_pca).mean(
        axis=2
    )  # (S, N_PCA) — mean |score| over T
    for indices, labels_grp, suffix in (
        (top_indices, top_labels, "top"),
        (bottom_indices, bot_labels, "bottom"),
    ):
    ...
    _logger.info(f"[{label}] IVA-frequency-channel: 20+ plot files saved to {out_dir}")
...
if __name__ == "__main__":
    ...
    _logger.info(
        f"IVA-frequency-channel: condition={condition.value}, "
    ...
    for mt in music_types:
        ...
        # Per-subject participant labels (3-digit ID from the metadata sidecar): the
        # mapping the stored components are indexed by. Only needed when storing, so
        # a run without --store_components is not held up by a missing sidecar.
        subject_ids = None
        if store_root is not None:
            subject_ids = participant_labels(
                getattr(analyzer, "filtered_df", None), ad.data.shape[0]
            )
