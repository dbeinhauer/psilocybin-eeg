...
:func:`src.io.iva_store.load_iva_components`. This variant puts *time* on the mixing
axis and channel × frequency on the sample axis, so what is stored is the component's
temporal pattern and its ``(frequencies, channels)`` score map — there is no
per-component TF map here; the channel-as-mixing variant
(``run_wavelet_iva_channel.py``) is the one that has one.
"""
...
_ANALYSIS_DIR = IvaVariants.TIME.value
...
def _run_iva(
    ...
    *,
    ...
    # The transposed companion of the frequency-channel variant: time is the
    # mixing axis, so a component owns a temporal pattern and a spectro-spatial
    # score map, and again there is no per-component time-frequency map.
    if store_root is not None:
        ...
        save_iva_components(
            ...
            arrays={
                IvaComponentArrays.TIMECOURSE: iva_components,
                IvaComponentArrays.FREQUENCY_CHANNEL_PATTERN: iva_scores,
            },
            freqs=freqs,
            times=time,
