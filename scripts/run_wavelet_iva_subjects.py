...
``notebooks/05-wavelet-iva-analysis/wavelet_iva_subjects.ipynb``
(subjects-as-datasets variant: K = S, features = C × F, samples = T;
per-subject PCA → IVA-G).
...
layout under::
    ...
    plots/05-wavelet-iva-analysis/<Condition>_<MusicType>/
        broadband/iva/                                # default (no --band)
            pca_scree_<label>.png
            ...
        bands/iva/                                    # when --band <name> is given
...
Usage::
    ...
    # broadband
    python scripts/run_wavelet_iva_subjects.py \\
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the subjects-as-datasets IVA decomposition on EEG wavelet "
            "power. Produces the same plots as wavelet_iva_subjects.ipynb, "
            "but for ALL IVA components."
        ),
...
# ---------------------------------------------------------------------------
# Main IVA pipeline (subjects-as-datasets)
# ---------------------------------------------------------------------------


def _run_iva_subjects(
    data_4d: np.ndarray,
    sfreq: float,
    freqs: np.ndarray,
    info,
    *,
    ...
    if band is None:
        out_dir = save_dir / "broadband" / "iva"
        prefix = ""
    else:
        out_dir = save_dir / "bands" / "iva"
        prefix = f"{band}_"
    out_dir.mkdir(parents=True, exist_ok=True)

    n_subjects, n_channels, n_freqs, n_times = data_4d.shape
    time = np.arange(n_times) / sfreq
    _logger.info(f"[{label}] IVA-subjects: {data_4d.shape}  sfreq={sfreq} Hz")
    ...
    source_corr = np.stack(
        [np.corrcoef(iva_sources_pca[:, k, :]) for k in range(n_show)]
    )
    _plot_isc_grid(
        source_corr,
    ...
    subj_time_view = iva_sources_pca[:, :n_show, :].transpose(1, 0, 2)  # (K, S, T)
    _plot_loo_isc_bar(
        subj_time_view,
        n_show=n_show,
        label=label,
    ...
    time_profiles = iva_sources_pca.mean(axis=0)[:n_show]  # (n_show, T)
    _plot_tf_map(
        freq_profiles,
        time_profiles,
        time,
    ...
    _logger.info(
        f"[{label}] IVA-subjects: 7 plot files saved to {out_dir}"
    )
...
if __name__ == "__main__":
    ...
    _logger.info(
        f"IVA-subjects: condition={condition.value}, "
