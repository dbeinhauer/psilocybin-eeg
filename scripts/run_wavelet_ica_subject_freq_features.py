...
and writes every plot into the canonical per-condition layout under::
    ...
    plots/04-subject-freq-features-wavelet-ica-analysis/<Condition>_<MusicType>/
        broadband/subject_freq_features/
...
Usage::
    ...
    python scripts/run_wavelet_ica_subject_freq_features.py \\
...
_STAGE_DIR = "04-subject-freq-features-wavelet-ica-analysis"
...
def _plot_isc_matrix(
    ...
    *,
    ...
    Uses per-subject frequency loading vectors: scores_2d shape (S, F, K).
    """
...
def _plot_time_frequency(
    ...
    *,
    ...
    """Time-frequency spectrograms for ALL ICs (cell 19)."""
    ...
    for i, ax in enumerate(axes):
        temporal_profile = component_time_profiles[i]
    ...
    axes[-1].set_xlabel("Time (s)")
    fig.suptitle(
        f"Time\u2013Frequency of Component Temporal Profiles \u2014 {label}",
        fontsize=13,
        y=1.01,
    )
    fig.tight_layout()
...
def _plot_subject_loadings(
    ...
    *,
    ...
    for i, ax in enumerate(axes):
        ax.barh(range(n_subjects), subject_loadings[:, i], color="seagreen")
        ax.set_yticks(range(n_subjects))
...
def _run_subject_freq_features(
    data_4d: np.ndarray,
    sfreq: float,
    freqs: np.ndarray,
    info,
    *,
    ...
    """Run the full subject-freq-features pipeline and save all plots."""
    out_dir = save_dir / "broadband" / "subject_freq_features"
    out_dir.mkdir(parents=True, exist_ok=True)

    n_subjects, n_channels, n_freqs, n_times = data_4d.shape
    time = np.arange(n_times) / sfreq
    _logger.info(f"[{label}] Subject-Freq Features: {data_4d.shape}  sfreq={sfreq} Hz")

    # Step 1 — Z-score and reshape: (S,C,F,T) → (S,F,C,T) → (S*F, C*T)
    bb_z = zscore_by_time(data_4d)
    bb_z_sf = bb_z.transpose(0, 2, 1, 3)  # (S, F, C, T)
    X_sf = bb_z_sf.reshape(n_subjects * n_freqs, n_channels * n_times)
    _logger.info(f"[{label}] Reshaped: {X_sf.shape}  (S*F, C*T)")

    # Step 2 — PCA + ICA
    pca = PCA(n_components=n_pca, random_state=random_state)
    pca_scores = pca.fit_transform(X_sf)
    _logger.info(
    ...
    # Reshape
    scores_2d = ica_scores.reshape(n_subjects, n_freqs, n_ica)  # (S, F, K)
    components_2d = ica_components.reshape(n_ica, n_channels, n_times)  # (K, C, T)
    _logger.info(
...
if __name__ == "__main__":
    ...
    _logger.info(
        f"Subject-Freq Features ICA: condition={condition.value}, "
    ...
    _logger.info("Subject-Freq Features ICA analysis complete.")
